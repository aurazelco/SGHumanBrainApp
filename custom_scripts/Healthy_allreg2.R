...
source("custom_scripts/global.R")
source("custom_scripts/Fun_Rank_disease.R")
source("custom_scripts/enrichr.R")
...
HealthyAllServer <- function(id) {
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$image <- renderImage({
      filename <- normalizePath(file.path(paste0('www/bulk/Healthy1.png')))
      list(
        src = filename, 
        height = 250
      )
