...
# 4. Function to filter out ns genes and too low FC, and order based on FC 
...
# 5. Function to filter the non-signifcant DEGs in all datasets
...
FilterDs <- function(list_ds, pval, FC) {
  ...
  for (group_id in names(list_ds)) {
    ...
    #sex_ls[["F"]] <- sex_ls[["F"]][lapply(sex_ls[["F"]],length)>0]
    #sex_ls[["M"]] <- sex_ls[["M"]][lapply(sex_ls[["M"]],length)>0]
...
# 6. Creates the df for the input ct so that we know if a DEG is found in a certain group or not -> used to generate hmps
...
# 7. Creates all PresenceDfs for all cts
...
# 8. Groups cts according to common annotation, then creates the presence dfs
...
# 9. Count DEGs for each ct in each age
...
# 10. Plots the total number of DEGs across groups, faceted by ct and sex
